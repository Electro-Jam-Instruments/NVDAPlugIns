...
> **Historical planning document.** Kept for the design reasoning. Build and release details
> are out of date: the version now lives only in `buildVars.py`, `addon/manifest.ini` is
> generated by scons and not in git, and releases go through `release.yml` via `/deploy`.
