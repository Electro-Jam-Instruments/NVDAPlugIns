...
> **Historical - do not follow.** This was the plan for the first 0.0.3 beta and is kept for
> reference only. The workflow it names (`build-addon.yml`) was replaced by `release.yml`, and
> `addon/manifest.ini` is now generated by scons from `buildVars.py` and is not in git.
> To release, use the `/deploy` command (`.claude/commands/deploy.md`).
